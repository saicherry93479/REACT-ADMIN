...
const Form = ({ post, data = [] }) => {
  ...
  useEffect(() => {
    if (post === "update" && data.length > 0) {
      ...
      setPhotos(photos);
      setVideo([{ ...data[0].video, update: "yes" }]);
    }
  }, []);
    ...
    if (uploadPhotos.length === photos.length && photos.length > 0) {
      ...
      if (b.length > 0) {
        ...
        if (video.length > 0) {
          ...
          videoUploader(video[0].file, video[0].id);
  ...
  const uploadToFireStore = async (data) => {
    ...
    const uniqueId = SeeRandom(data);
    console.log("unique id is ", uniqueId);
    data.uniqueId = uniqueId;
    console.log("data after unique id  is ", data);
  ...
  const videoUploader = async (item, _id) => {
    if (item.update === "no") {
      console.log("file video is already presnt url is ", item.imageUrl);
      console.log("file is in videoUploader ", item.file.name);
      const name = `videos/${file.name}`;
      const ref = storageRef(storage, name);
      try {
        const uploadTask = await uploadBytesResumable(ref, item.file);
        console.log("uploadTask is ", uploadTask);
  ...
  const uploadHandler = () => {
    ...
    if (errorChecker()) {
      //uploading files ################
      setUploadStart(true);
      photos.map((item) => imageUploader(item.file, item.id));
    } else {
